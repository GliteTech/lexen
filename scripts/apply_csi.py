...
def label(keys: list[str], m: dict[str, str]) -> dict:
    return {
...
def cand_mappings(keys: list[str], m: dict[str, str]) -> list[dict]:
    return [{"concept_id": concept_for(k, m), "mapped": k in m, "sense_key": k}
            for k in sorted(set(keys))]


def record(item: dict, m: dict[str, str], mapping_id: str, schema_version: str) -> dict:
...
def main() -> None:
    ...
    glite = load_mapping(GLITE_DIR / "wordnet_sense_key_to_glite_concept.jsonl",
                         GLITE_DIR / "lexen_report_aliases.json")
    csi = load_mapping(CSI_DIR / "wordnet_sense_key_to_csi_concept.jsonl",
                       CSI_DIR / "csi_report_aliases.json")
    ...
    print(f"wrote {SIDECAR} ({len(items)} items; {n_unmapped} items have >=1 unmapped candidate under CSI)")
